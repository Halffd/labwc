	...
	int *width_out, int *height_out)
{
	...
	if (!buffer) {
		wlr_log(WLR_ERROR, "failed to allocate buffer for screenshot");
		return false;
	}
...
static bool
capture_fullscreen(struct wlr_buffer **buffer_out, int *width_out, int *height_out)
{
	/* Calculate total bounds of all outputs */
	int total_width = 0;
	int total_height = 0;
...
capture_region(int x, int y, int width, int height,
	struct wlr_buffer **buffer_out, int *width_out, int *height_out)
{
	struct wlr_buffer *buffer = wlr_allocator_create_buffer(
		server.allocator, width, height, WL_SHM_FORMAT_ARGB8888);
	if (!buffer) {
		wlr_log(WLR_ERROR, "failed to allocate buffer for screenshot");
		return false;
	}
...
{
	struct screenshot_ipc_message msg;
	...
	if (msg.magic != SCREENSHOT_IPC_MAGIC) {
		close(client_fd);
		return;
	}
	...
	if (!runtime_dir) {
		strcpy(response, "ERROR: XDG_RUNTIME_DIR not set");
	...
	switch (msg.command) {
	case SCREENSHOT_IPC_FULL: {
		struct wlr_buffer *buffer;
		int width, height;
		if (!capture_fullscreen(&buffer, &width, &height)) {
			strcpy(response, "ERROR: capture failed");
			response_len = strlen(response);
			break;
		}
		snprintf(screenshot_path, sizeof(screenshot_path),
			"%s/labwc-screenshot-full-%ld.png", runtime_dir, (long)time(NULL));
		save_png_from_buffer(buffer, width, height, screenshot_path);
		wlr_buffer_drop(buffer);
		snprintf(response, sizeof(response), "OK:%s", screenshot_path);
		response_len = strlen(response);
		break;
	}
	case SCREENSHOT_IPC_OUTPUT: {
		if (payload) {
			struct output *o;
		...
		if (!output) {
			strcpy(response, "ERROR: no output found");
			response_len = strlen(response);
			break;
		}
		struct wlr_buffer *buffer;
		...
		snprintf(screenshot_path, sizeof(screenshot_path),
			"%s/labwc-screenshot-%s-%ld.png", runtime_dir,
			output->wlr_output->name, (long)time(NULL));
		save_png_from_buffer(buffer, width, height, screenshot_path);
		wlr_buffer_drop(buffer);
		snprintf(response, sizeof(response), "OK:%s", screenshot_path);
		response_len = strlen(response);
	...
	case SCREENSHOT_IPC_REGION: {
		if (!payload) {
			strcpy(response, "ERROR: no region specified");
			response_len = strlen(response);
			break;
		}
		int rx, ry, rw, rh;
		if (sscanf(payload, "%d,%d,%d,%d", &rx, &ry, &rw, &rh) != 4) {
			strcpy(response, "ERROR: invalid region format (x,y,w,h)");
			response_len = strlen(response);
			break;
		}
		struct wlr_buffer *buffer;
		int width, height;
		if (!capture_region(rx, ry, rw, rh, &buffer, &width, &height)) {
			strcpy(response, "ERROR: capture failed");
			response_len = strlen(response);
			break;
		}
		snprintf(screenshot_path, sizeof(screenshot_path),
			"%s/labwc-screenshot-region-%ld.png", runtime_dir, (long)time(NULL));
		save_png_from_buffer(buffer, width, height, screenshot_path);
		wlr_buffer_drop(buffer);
		snprintf(response, sizeof(response), "OK:%s", screenshot_path);
		response_len = strlen(response);
	...
	case SCREENSHOT_IPC_FOCUSED: {
		struct wlr_buffer *buffer;
		int width, height;
		if (!capture_focused(&buffer, &width, &height)) {
			strcpy(response, "ERROR: no focused window");
			response_len = strlen(response);
			break;
		}
		snprintf(screenshot_path, sizeof(screenshot_path),
			"%s/labwc-screenshot-focused-%ld.png", runtime_dir, (long)time(NULL));
		save_png_from_buffer(buffer, width, height, screenshot_path);
		wlr_buffer_drop(buffer);
		snprintf(response, sizeof(response), "OK:%s", screenshot_path);
		response_len = strlen(response);
	...
	case SCREENSHOT_IPC_LIST_OUTPUTS: {
		char *p = response;
...
void
screenshot_ipc_init(void)
{
	const char *runtime_dir = getenv("XDG_RUNTIME_DIR");
	if (!runtime_dir) {
		wlr_log(WLR_ERROR, "XDG_RUNTIME_DIR not set, cannot create screenshot IPC socket");
		return;
	}

	snprintf(socket_path, sizeof(socket_path), "%s/labwc-screenshot.sock", runtime_dir);

	ipc_fd = socket(AF_UNIX, SOCK_STREAM, 0);
	if (ipc_fd < 0) {
		wlr_log(WLR_ERROR, "Failed to create screenshot IPC socket");
		return;
	}
	...
	if (bind(ipc_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		wlr_log(WLR_ERROR, "Failed to bind screenshot IPC socket to %s", socket_path);
		close(ipc_fd);
		ipc_fd = -1;
		return;
	}
	...
	if (listen(ipc_fd, 5) < 0) {
		wlr_log(WLR_ERROR, "Failed to listen on screenshot IPC socket");
		close(ipc_fd);
		ipc_fd = -1;
		unlink(socket_path);
		return;
	...
	if (!ipc_event_source) {
		wlr_log(WLR_ERROR, "Failed to add screenshot IPC event source");
		close(ipc_fd);
		ipc_fd = -1;
		unlink(socket_path);
		return;
	...
	wlr_log(WLR_INFO, "Screenshot IPC listening on %s", socket_path);
}

void
screenshot_ipc_finish(void)
{
	ipc_close();
}